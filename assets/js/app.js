...
const authorDirectory = {
  ...
  "Jinyi Li": "https://dblp.org/search/author?q=Jinyi%20Li",
...
function renderProfile(profile) {
  ...
  $("#bio-heading").textContent = t("bioKicker");
  ...
  $("#education-list").innerHTML = (profile.education || [])
    .map(
      (item) => `
        <li class="detail-item education-item">
          <div class="education-line">
            <span class="education-years">${esc(item.years)}</span>
            <span class="education-degree">${esc(item.degree)}</span>
          </div>
          <div class="detail-item-copy education-school">${esc(item.school)}</div>
        </li>
      `
    )
    .join("");
...
function visiblePublications() {
  const labels = publicationLabelMap();
  return state.publications
    .filter((pub) => matchesQuery(pub) && matchesType(pub))
    .sort((a, b) => {
      const yearDiff = (Number(b.year) || 0) - (Number(a.year) || 0);
      if (yearDiff) return yearDiff;
      const typeDiff = (typeOrder[a.type] ?? 99) - (typeOrder[b.type] ?? 99);
      if (typeDiff) return typeDiff;
      const aLabel = labels.get(a.id) || {};
      const bLabel = labels.get(b.id) || {};
      if (aLabel.code === bLabel.code && aLabel.num !== bLabel.num) {
        return (bLabel.num || 0) - (aLabel.num || 0);
      }
...
function renderMetrics(meta) {
  ...
  if (footerYear) footerYear.textContent = `${t("updated")} ${updatedAt}`;
...
function publicationLabelMap() {
  const counts = {};
  const labels = new Map();
  for (const pub of chronologicalPublications()) {
    const code = typeCode[pub.type] || "o";
    counts[code] = (counts[code] || 0) + 1;
    labels.set(pub.id, { text: `${code}${counts[code]}`, code, num: counts[code] });
  }
  return labels;
}

function yearGroupMarkup(year, items, labels) {
  const rows = items
    .map((pub) => {
      const label = (labels.get(pub.id) || {}).text || "";
      ...
      const citationText = Number.isFinite(Number(pub.citationCount)) ? ` <span class="dblp-citations">${pub.citationCount} ${esc(t("citations"))}</span>` : "";
      const jcrText = pub.jcrQuartile ? ` <span class="jcr-badge">${esc(pub.jcrQuartile)}</span>` : "";
      return `
        <article class="dblp-row">
          <div class="dblp-marker ${colorClass}"></div>
          <div class="dblp-label">[${label}]</div>
          <div class="dblp-entry">
            <div class="dblp-titleline">
              <span class="dblp-authors">${authorMarkup(pub.authors || [])}:</span>
              <span class="dblp-title">${title}</span>
            </div>
            <div class="dblp-venue">
              <em>${esc(venueText(pub))}</em> (${esc(pub.year)})${citationText}${jcrText}
            </div>
          </div>
        </article>
      `;
...
function renderPublications() {
  ...
  list.innerHTML = Array.from(grouped.entries())
    ...
    .map(([year, items]) => yearGroupMarkup(year, items, labels))
    .join("");
}
