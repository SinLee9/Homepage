...
const strings = {
  en: {
    ...
    refineCoauthor: "refine by coauthor",
    ...
    .map(
      (item) => `
        <li class="education-card">
          <div class="education-years">${esc(item.years)}</div>
          <div class="education-copy">
            <div class="education-degree">${esc(item.degree)}</div>
            <div class="education-place">${esc(item.school)}</div>
          </div>
        </li>
      `
    )
...
function renderRefineSidebar() {
  ...
  coauthorContainer.innerHTML = [
    `<button class="refine-link${!state.filters.coauthor ? " is-active" : ""}" type="button" data-coauthor="">${esc(t("dismissAll"))}</button>`,
    ...coauthors.map(
      ([value, count]) =>
        `<button class="refine-link${state.filters.coauthor === value ? " is-active" : ""}" type="button" data-coauthor="${esc(value)}">${esc(value)} <span>(${count})</span></button>`
    )
  ].join("");
  ...
  venueContainer.innerHTML = [
    ...
    ...venues.map(
      ([value, count]) =>
        `<button class="refine-link${state.filters.venue === value ? " is-active" : ""}" type="button" data-venue="${esc(value)}">${esc(value)} <span>(${count})</span></button>`
    )
  ].join("");
  ...
  $$("[data-coauthor]").forEach((button) => {
    button.addEventListener("click", () => {
      state.filters.coauthor = button.dataset.coauthor;
      renderRefineSidebar();
      renderPublications();
    });
  });
