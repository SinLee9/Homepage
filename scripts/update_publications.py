...
- Google Scholar search links, used as public navigation links.
- data/manual_publications.json, used for curated manuscripts and local notes.
"""
...
def merge_records(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # Manual records are applied last as curated overrides.
    for manual in load_manual():
        key = key_for(manual)
        target = by_key.get(key)
        if not target:
            manual.setdefault("sources", ["Manual"])
            by_key[key] = manual
            continue
    ...
    return sorted(items, key=lambda item: (-int(item.get("year") or 0), item.get("title", "")))
